...
locals {
  ...
  # Callers can override via var.bucket_policy_enabled when custom_policy is known-after-apply
  # (otherwise the boolean cannot be determined at plan time).
  create_bucket_policy = var.bucket_policy_enabled != null ? var.bucket_policy_enabled : (length(var.policy_templates) > 0 || var.custom_policy != null)
}
