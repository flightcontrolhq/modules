...
run "manually_split_long_value_succeeds" {
  ...
  assert {
    condition     = aws_route53_record.this["TXT-selector._domainkey.example.com"].records == toset(["v=DKIM1;k=rsa;p=01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789\" \"01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789"])
    error_message = "A manually split TXT record should retain the AWS provider's internal quote separator"
  }
}

run "surrounding_quotes_on_manually_split_txt_fail" {
  command = plan
    ...
    records = [
      {
        ...
        type    = "TXT"
        ttl     = 300
        records = ["\"first-part\" \"second-part\""]
      }
    ]
  }
