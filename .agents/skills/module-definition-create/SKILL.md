...
Use these as naming and identifier baselines.
...
2. Search this repo for existing `*-definition.yml` files and verify no active definition already uses the requested `definition.type`.
3. Normalize the proposed definition fields: title-case service names, keep `definition.type` lowercase kebab-case with the `rvn-` prefix for first-party Ravion modules, and keep descriptions concise.
4. Add `<definition.type>-definition.yml` beside the Terraform module using the top-level `definition`, `release`, and `module` sections.
5. Add only a minimal `module` shell unless the user also asked for config or README content.
6. Validate the authored definition with the module publishing tool.
