...
# Target Groups
#
# A production (tg-1) + alternate (tg-2) pair is always created when a
# load balancer is attached, regardless of deployment strategy. This
# keeps the deployment strategy a pure per-deployment decision: the
# deploy manager can switch between rolling / blue_green / linear /
# canary on any UpdateService call without Terraform changes. Rolling
# deployments simply serve from the production target group and never
# touch the alternate.
################################################################################
