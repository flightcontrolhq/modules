...
resource "aws_launch_template" "host" {
  ...
  user_data = base64encode(templatefile("${path.module}/templates/user-data.sh.tpl", {
    ...
    # Empty when TCP exposure is off, which is exactly how the host refuses a
    # mode=tcp port request instead of publishing one that answers nothing.
    tcp_target_group_arn = var.enable_tcp_exposure ? aws_lb_target_group.tcp[0].arn : ""
