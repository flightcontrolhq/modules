...
#
# OFF BY DEFAULT, AND NO LONGER RAVION'S LOG PIPELINE. Logs come from Loki
# (loki.tf / alloy.tf) and metrics from Amazon Managed Prometheus
# (amp.tf / otel_collector.tf); this add-on stays only because Container
# Insights is a thing customers may already depend on, or may simply want. It
# duplicates both halves of Ravion's own pipeline at CloudWatch prices, so
# enabling it alongside them is a deliberate choice, not a default.
################################################################################
