################################################################################
# S3 Bucket with ALB Access Logs Policy Fixture
#
# Creates an S3 bucket with ALB access logs policy template for Terratest
# integration testing. This fixture tests the policy_templates feature with
# both 'alb_access_logs' and 'deny_insecure_transport' templates.
################################################################################
...
################################################################################
# S3 Bucket with ALB Access Logs Policy
################################################################################
...
module "s3_bucket" {
  ...
  # Apply ALB access logs and deny insecure transport policy templates
  policy_templates = ["alb_access_logs", "deny_insecure_transport"]

  # Include lifecycle rule for log expiration (matches README example)
  lifecycle_rules = [
    {
      id = "expire-logs"
      expiration = {
