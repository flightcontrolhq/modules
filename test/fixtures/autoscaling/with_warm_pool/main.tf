################################################################################
# Auto Scaling Group with Warm Pool Fixture
#
# Demonstrates warm pool configuration for faster instance scaling.
# Pre-initializes instances in a stopped state for quick launch when needed.
# Includes instance reuse policy to return instances to warm pool on scale-in.
#
...
module "autoscaling" {
  ...
  warm_pool = {
    # Instances in warm pool are stopped (cost-effective)
    pool_state = "Stopped"

    # Minimum instances to maintain in warm pool
    min_size = 1

    # Maximum total prepared capacity (in warm pool + pending)
  ...
  # Enable CloudWatch metrics including warm pool metrics
  enabled_metrics = [
    "GroupMinSize",
    "GroupMaxSize",
