...
locals {
  ...
  create_parameter_group = var.parameter_group_enabled
  create_option_group    = var.option_group_enabled && (local.is_oracle || local.is_sqlserver)
  create_monitoring_role = var.monitoring_role_enabled && var.monitoring_interval > 0

  # Read replica creation
  create_read_replicas = var.read_replica_enabled && var.read_replica_count > 0
