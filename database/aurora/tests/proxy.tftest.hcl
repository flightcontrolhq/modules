...
mock_provider "aws" {
  ...
  # The proxy security group ID feeds the cluster security group's ingress
  ...
  override_resource {
    ...
    values = {
      arn = "arn:aws:iam::123456789012:role/test-aurora-rds-proxy"
    }
  }
...
# graph, including the proxy module's depends_on on cluster instances.
run "test_proxy_created_when_enabled" {
  ...
  assert {
    ...
    error_message = "aurora-postgresql should map to the POSTGRESQL engine family."
  }
}

run "test_proxy_engine_family_mysql" {
  command = plan
  ...
  assert {
    ...
    error_message = "aurora-mysql should map to the MYSQL engine family."
  }
}
...
run "test_proxy_requires_secret_when_password_management_disabled" {
  ...
  variables {
    ...
    master_password                         = "dummy-password-123"
  }
